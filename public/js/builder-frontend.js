(function ($) {
    ...
    var ShaBuilder = {
        ...
        populatePropertiesPanel: function ($el) {
            ...
            $.each(style.split(';'), function (i, s) {
                var parts = s.split(':');
                if (parts.length === 2) styleObj[$.trim(parts[0])] = $.trim(parts[1]);
            });
            var cssFields = [
                { p: 'width', t: 'text' }, { p: 'height', t: 'text' },
                { p: 'margin', t: 'text' }, { p: 'padding', t: 'text' },
                { p: 'color', t: 'text' }, { p: 'background-color', t: 'text' },
                { p: 'background-image', t: 'text' },
                { p: 'font-size', t: 'text' }, { p: 'font-weight', t: 'text' },
                { p: 'font-family', t: 'text' }, { p: 'text-align', t: 'select', o: ['left', 'center', 'right', 'justify'] },
                { p: 'display', t: 'select', o: ['block', 'inline', 'inline-block', 'flex', 'grid', 'none'] },
                { p: 'position', t: 'select', o: ['static', 'relative', 'absolute', 'fixed', 'sticky'] },
                { p: 'border', t: 'text' }, { p: 'border-radius', t: 'text' },
                { p: 'box-shadow', t: 'text' },
                { p: 'opacity', t: 'text' }, { p: 'transform', t: 'text' },
                { p: 'overflow', t: 'select', o: ['visible', 'hidden', 'scroll', 'auto'] }
            ];
            var styleHtml = '<div class="sha-props-group"><div class="sha-props-group-title">CSS Properties</div>';
            $.each(cssFields, function (i, f) {
                var val = styleObj[f.p] || '';
                if (f.t === 'select') {
                    var opts = '';
                    $.each(f.o, function (j, o) {
                        opts += '<option value="' + o + '"' + (val === o ? ' selected' : '') + '>' + o + '</option>';
                    });
                    styleHtml += '<label class="sha-props-field">'
                        + '<span>' + f.p + '</span>'
                        + '<select data-css="' + f.p + '">' + opts + '</select>'
                        + '</label>';
                } else {
                    styleHtml += '<label class="sha-props-field">'
                        + '<span>' + f.p + '</span>'
                        + '<input type="text" data-css="' + f.p + '" value="' + ShaBuilder.escHtml(val) + '" />'
                        + '</label>';
                }
            });
            styleHtml += '</div>';
            styleHtml += '<div class="sha-props-group"><div class="sha-props-group-title">Custom CSS</div>'
                + '<textarea class="sha-props-custom-css" id="sha-props-custom-css" placeholder="Any valid CSS rules">' + ShaBuilder.escHtml(style) + '</textarea>'
                + '</div>';
            $('#sha-props-style').html(styleHtml);

            /* ---- effects tab ---- */
            var effectsHtml = '<div class="sha-props-group"><div class="sha-props-group-title">Transforms</div>';
            var transform = styleObj['transform'] || '';
            effectsHtml += '<label class="sha-props-field"><span>Scale</span><input type="range" min="0.1" max="3" step="0.1" value="1" id="sha-eff-scale" /><span class="sha-eff-val" id="sha-eff-scale-val">1</span></label>';
            effectsHtml += '<label class="sha-props-field"><span>Rotate (deg)</span><input type="range" min="-360" max="360" step="1" value="0" id="sha-eff-rotate" /><span class="sha-eff-val" id="sha-eff-rotate-val">0</span></label>';
            effectsHtml += '</div>';
            effectsHtml += '<div class="sha-props-group"><div class="sha-props-group-title">Filters</div>';
            effectsHtml += '<label class="sha-props-field"><span>Blur (px)</span><input type="range" min="0" max="20" step="0.5" value="0" id="sha-eff-blur" /><span class="sha-eff-val" id="sha-eff-blur-val">0</span></label>';
            effectsHtml += '<label class="sha-props-field"><span>Brightness</span><input type="range" min="0" max="3" step="0.1" value="1" id="sha-eff-brightness" /><span class="sha-eff-val" id="sha-eff-brightness-val">1</span></label>';
            effectsHtml += '<label class="sha-props-field"><span>Contrast</span><input type="range" min="0" max="3" step="0.1" value="1" id="sha-eff-contrast" /><span class="sha-eff-val" id="sha-eff-contrast-val">1</span></label>';
            effectsHtml += '</div>';
            effectsHtml += '<div class="sha-props-group"><div class="sha-props-group-title">Box Shadow</div>';
            effectsHtml += '<label class="sha-props-field"><span>H-offset</span><input type="text" id="sha-eff-sh-h" value="0" placeholder="0" /></label>';
            effectsHtml += '<label class="sha-props-field"><span>V-offset</span><input type="text" id="sha-eff-sh-v" value="0" placeholder="0" /></label>';
            effectsHtml += '<label class="sha-props-field"><span>Blur</span><input type="text" id="sha-eff-sh-blur" value="10" placeholder="10" /></label>';
            effectsHtml += '<label class="sha-props-field"><span>Color</span><input type="text" id="sha-eff-sh-color" value="rgba(0,0,0,0.3)" placeholder="rgba(0,0,0,0.3)" /></label>';
            effectsHtml += '</div>';
        ...
        applyPropsToElement: function ($el) {
            ...
            /* style tab: individual fields */
            var rules = [];
            $('#sha-props-style input[data-css]').each(function () {
                var prop = $(this).data('css');
                var val = $(this).val();
                if (val) rules.push(prop + ':' + val);
            });
            $('#sha-props-style select[data-css]').each(function () {
                var prop = $(this).data('css');
                var val = $(this).val();
                if (val) rules.push(prop + ':' + val);
            });
            ...
            var blur = parseFloat($('#sha-eff-blur').val());
            var brightness = parseFloat($('#sha-eff-brightness').val());
            var contrast = parseFloat($('#sha-eff-contrast').val());
            var filters = [];
    ...
    // Element inspection: click to select
    $(document).on('click.shaInspect', function (e) {
        if (!ShaBuilder._inspectActive) return;
        ShaBuilder.selectElement($(e.target));
    });
